...
await fastify.register(fastifySwagger, {
  openapi: {
    openapi: '3.0.3',
    info: {
      title: 'Lab 9 API — Students (JWT auth з access/refresh + blacklist)',
      description: 'Lab_9_JWT: @fastify/jwt; access (15m) у Authorization: Bearer; refresh (7d) у httpOnly cookie; blacklist через trusted callback.',
      version: '1.0.0',
    },
    servers: [{ url: `http://localhost:${process.env.PORT || 3000}` }],
    tags: [
      { name: 'health',  description: 'Перевірка стану сервера' },
      { name: 'auth',    description: 'Реєстрація / вхід / refresh / вихід (JWT)' },
      { name: 'items v1', description: 'Студенти — CRUD захищено JWT (Authorization: Bearer)' },
